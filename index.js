...
const prefix = ".";
const ownerNumber = [config.BOT_OWNER]; // ✅ emoji අයින්
...
async function ensureSessionFile() {
  if (!fs.existsSync(credsPath)) {
    ...
    // Full URL or just file key
    let url = config.SESSION_ID.startsWith("http")
      ? config.SESSION_ID
      : `https://mega.nz/file/${config.SESSION_ID}`;
    ...
    try {
      ...
      console.error("❌ Invalid SESSION_ID provided.", e);
      process.exit(1);
    }
  } else {
    setTimeout(() => {
...
async function connectToWA() {
  console.log("Connecting DINETH_MD 🧬...");
  ...
  danuwa.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      console.log("✅ DINETH_MD connected to WhatsApp");

      const up = `DINETH_MD connected ✅\n\nPREFIX: ${prefix}`;
      await danuwa.sendMessage(ownerNumber[0] + "@s.whatsapp.net", {
        image: { url: `https://files.catbox.moe/qqchow.jpg` },
        caption: up,
      });
...
app.get("/", (req, res) => {
  res.send("Hey, DINETH_MD started✅");
});
